...
const completedLog: WorkoutLog = {
  ...
  workout: baseWorkout,
  exercises: [
    { id: 'el1', exercise: baseWorkout.exercises[0], sets: [], completedAt: new Date() },
    { id: 'el2', exercise: baseWorkout.exercises[1], sets: [], completedAt: new Date() },
    { id: 'el3', exercise: baseWorkout.exercises[2], sets: [], completedAt: new Date() },
  ],
...
describe('WorkoutProgressCard', () => {
  it('renders workout name and description', () => {
    const { getByText } = render(
      <WorkoutProgressCard workout={baseWorkout} log={undefined} onPress={jest.fn()} onCheck={jest.fn()} />,
    );
    expect(getByText('Upper Body Power')).toBeTruthy();
    expect(getByText('Focus on progressive overload for chest and back.')).toBeTruthy();
  });
  ...
  it('shows 1 of 3 exercises completed for partial log', () => {
    const { getByText } = render(
      <WorkoutProgressCard workout={baseWorkout} log={partialLog} onPress={jest.fn()} onCheck={jest.fn()} />,
    );
  ...
  it('shows 3 of 3 exercises completed when all done', () => {
    const { getByText } = render(
      <WorkoutProgressCard workout={baseWorkout} log={completedLog} onPress={jest.fn()} onCheck={jest.fn()} />,
    );
  ...
  it('does not render description when absent', () => {
    ...
    const { queryByTestId } = render(
      <WorkoutProgressCard workout={noDesc} log={undefined} onPress={jest.fn()} onCheck={jest.fn()} />,
    );
  ...
  it('calls onCheck when checkbox pressed', () => {
    ...
    const { getByTestId } = render(
      <WorkoutProgressCard workout={baseWorkout} log={undefined} onPress={jest.fn()} onCheck={onCheck} />,
    );
  ...
  it('calls onPress when card pressed', () => {
    ...
    const { getByTestId } = render(
      <WorkoutProgressCard workout={baseWorkout} log={undefined} onPress={onPress} onCheck={jest.fn()} />,
    );
